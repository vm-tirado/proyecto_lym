...
public class MundoParsers {
	...
	public  MundoParsers () {
	    ...
	    parsers.add("Proyecto");
	    
	    currentParser =  0;

	}
	
	public Proyecto getParserProyecto(){
		return new Proyecto(System.in);
	}
	...
	public String  procesarCadena(String texto) {
		...
		else if(parsers.get(currentParser).equals("Proyecto")){
			Proyecto nuevoParser = getParserProyecto();
			Proyecto.ReInit(new java.io.StringReader(texto));
			try {
		    	Proyecto.one_line(); 
		    	resp = new String("OK    \n");
		    }catch (Exception e) {
